...
#### deveguru
  .git didrc leakage, gittool to get the git file by git dumper to download and git extractor to restore to normal files; get the db credential; login web server backend; htm file RCE method, wget reverse shell to load it via web browser; searching for abnormal filistening on [any] 8821 ...
...
ben:x:1000:1000:,,,:/home/ben:/bin/bashle - backups; get gitea db credential; gitea githook to launch hook shell(system bash shell); sudo -l sqlite3 but without passwd; sudo version , vuln; sudo -u#-1;
#### tommyboy
...
#### intelligence.htb
  port 80, enum naming stype of pdf files, get default passwd, and user list；use kerbrute to validate user, and check smb files, get user flag, and a ps script; add dns entry to AD,use responsor catch a hash of a user; use bloodhound find a exploit path, ReadGMSAPassword, allowtoDelegate, get domain

#### fuse.htb
  smb,rpc enum, smbpasswd reset, get more credential, one canbe used to remote login; SeLoadDriverPrivilege to es

#### scrambled.htb
  http enum, get a user, and potuantial passwd same as username, NTLM auth is disabled; use this use access SMB, get a pdf file, Network admin group is able to access MSSQL; GetUserSPNs to enum SPN, get ahash of sqlsvc, crack it; enum SID of dimain, and generate NTLM of the new cracked credential, use these to access MSSQL, get another credential; MSSQL use cmdshell to download nc to target host, and get shell of the previous user; bloodhound view the privi of the previous user, move to the last user and get ashell, and user flag; enum the PE file and dll on share folder, which is used to connect with port 4411, debug the exe on Winx64, get a log file, which is the same format as NC to the port, use ysoserial on win to generate a revershell payload and nc to trigger it, get SyStem shell

#### cap.htb
  http enum, get a username, fuzz the data folder by id, 0 download file valides passwd for this user; ssh to host, linpeas view python with setuid, ES

#### codeparttwo.htb
  port 8000 running python based JS interprater, with a vuln to RCE, get shell; check the SQL db file, find user credetnial, ssh in host; sudo -l with ALL to a binary file, 
#### expressway.htb
  port 500 ike enum to get user credential; sudo version vuln get root

#### facs.htb
  vuln of Camalieon ES and system data leakage, view the system files, and get Aws keys; Aws connect Aws bucket get a private key, crack it by john; ssh into host; sudo -l, facter

#### Converso.htb
  XSLT injection, write revershell to host, get a shell; sqlite view .db get credential; sudo -l 

#### soulmate.htb
  ffuf the subdomain, get ftp, running crushftp has vunl, exploit to add a admin user; with this account to reset passwd for a user who runs the web applicaiton, and add a revershell to the web dir, trigger to get a www-data shell; linpeas find a script with user credential, ssh user; test local port 2222, the applicaiton has vunl, also nc it find os module is used, execute os command to launch revershell by root privi

#### cctv.htb
  port 80 service with vuln to sqli, slqmap get a user credential; ssh to host, check listening port, enum the service , get admin and apsswd hash; forfprwarding to local, to visit and get application info and versionl; it is vuln to RCE, get root shell

#### principal.htb
  port 80 vuln to pub key bypass, use poc to login the servie as admin; find user list and a potential passwd; nxc to spray the user, get user; sshd misconfig, trustCA is set, AuthorizedPrincipalsFile or AuthorizedPrincipalsCommand not set. use ca to sign our own pub key with pricipal as root to ssh in as root.

#### kobold.htb
  port 80, 2 subdomains, one of it vuln to RCE, get a user shell, but cannot lateral move or ES; 2nd subdomain, RCE get a shell, inside a container, find a credential, reuse it in port 3552; container management system, mount / of host to /app of the container disk

#### devarea.htb
  port 21, get a jar file, got a service; vuln of service running on 8080, FLI the service previous, got a credential for port 8888; vuln RCE of port 8888, get user; /bin/bash is writable, sudo -l to bash hijack get root

#### 2million
  port 80, js file, get API info;API Enum register account and set to admin privi; use VPN/generate API to code injection command get shell;kernel vuln to root

#### Pilgrimage.htb
  .git folder, gittool; hex string to binary , xxd -r -p;

#### keeper.htb
  .kdbx use kpcli to access; putty user key file, use puttygen the .key file to convert to private-openssh format id_rsa
